...
import Network.Topologies.Topology6Simple;
import Network.Topologies.TopologyIRoutes;
import Network.Topologies.TopologyGeneral;
import Network.Topologies.TopologyNSFNet;
import Network.Topologies.TopologyOneLink;
import Network.Topologies.TopologyOneLinkOnly;
...
public class TopologyManager {
    ...
    public void inicialize() throws Exception {
        ...
        if (ParametersSimulation.getTopologyType().equals(TopologyType.NSFNet)){
            ...
            if (ParametersSimulation.getTopologyType().equals(TopologyType.OneLink)){
                NetworkTopologyInstance = new TopologyOneLink();
            } else {
                if (ParametersSimulation.getTopologyType().equals(TopologyType.OneLinkOnly)){
                    NetworkTopologyInstance = new TopologyOneLinkOnly();
                } else {
                    if (ParametersSimulation.getTopologyType().equals(TopologyType.Simple6)){
                        NetworkTopologyInstance = new Topology6Simple();
                    } else {
                        if (ParametersSimulation.getTopologyType().equals(TopologyType.IRoutes)){
                            NetworkTopologyInstance = new TopologyIRoutes();
                        } else {
    ...
    public OpticalLink[][] getNetworkOpticalLinks() {
        return networkOpticalLinks;
    }
