...
public class CallRequestManager {
    ...
    public void removeCallRequestByMSCL(double acumulatedTime) {
        ...
        while(iterator.hasNext()){
			...
			// Perda de capacidade com a ausência dessa requisição
			

			if(time < acumulatedTime){

				// Procura outra requisição do o mesmo par origem destino
				for (CallRequest possibleCallRequest : this.callReqList) {
					if(possibleCallRequest.getSourceNodeID() == callRequest.getSourceNodeID() && possibleCallRequest.getDestinationNodeID() == callRequest.getDestinationNodeID() && possibleCallRequest.getDecayTime() >= acumulatedTime){
						System.out.println("Achou uma requisição com o mesmo par origem destino");
					}
				}
